	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		if (successor != null) {
			...
			if (Util.checkInterval(key, nodeId.add(BigInteger.ONE), succId)) {
				return successor; // Key is within the range of this node and its successor
			} else {
				// Key is not within the range, find the highest predecessor
				NodeInterface highestPredecessor = findHighestPredecessor(key);

				if (highestPredecessor != null) {
					// Recursively call findSuccessor on the highest predecessor found
					return highestPredecessor.findSuccessor(key);
				} else {
					logger.warn("Failed to find successor for key: " + key);
				}
			}
		}

		return null; // No valid successor found
	}
		...
		for (int i = fingerTable.size() - 1; i >= 0; i--) {
			...
			if (fingerNode != null) {
				...
				if (Util.checkInterval(fingerID, node.getNodeID().add(BigInteger.ONE), ID)) {
					// Check if the finger node is a predecessor candidate for the given key
					return fingerNode;
				}
			}
		}

		// If no valid predecessor is found in the finger table, return the closest preceding node
		NodeInterface closestPredecessor = node.getPredecessor();
		if (closestPredecessor != null) {
			return closestPredecessor;
		}

		return null; // No valid predecessor found
	}
