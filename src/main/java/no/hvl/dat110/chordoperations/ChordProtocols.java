...
public class ChordProtocols {
	...
	public void fixFingerTable() {
		...
		try {
			...
			List<NodeInterface> fingerTabel = chordnode.getFingerTable();
			fingerTabel.clear();
			...
			for(int i = 0; i < numBits; i++){
				...
				BigInteger k = nodeId.add(exponent).mod(addressSize);

				NodeInterface succsessorNode = chordnode.findSuccessor(k);

				if(succsessorNode != null){
					fingerTabel.add(succsessorNode);
				} else {
					logger.warn("Failed to find succsessor for " + k);
				}
			}
			...
			logger.info("Error while fixing FingerTable: " + e.getMessage());
		}
	}
